...
# Methods: SOS + Bin 2nd stage + no good cut + callback cut (if necessary) + UB + critical buses + scenario dominance
...
include("second_stage.jl") #second stage Binary version
...
for s=1:sizew #s for scenariow
     if fw2nd[s]==0
     fw2nd[s]=f2nd_bin(scenariow[:,:,s],aiwtscen[:,:,s],mpcmodel)
